defmodule LiquidCrystal.HD44780.I2CTest do
  ...
  test "LiquidCrystal.I2C mock works" do
    assert {:ok, i2c_ref} = LiquidCrystal.I2C.open("i2c-1")
    assert :ok = LiquidCrystal.I2C.write(i2c_ref, 0x20, <<0x00, 0x0F>>)
  end
  ...
  describe "commands" do
    ...
    test "execute valid commands", %{display: d} do
      ...
      assert {:ok, %{}} = HD44780.I2C.execute(d, {:display, :off})
      assert {:ok, %{}} = HD44780.I2C.execute(d, {:display, :on})
      assert {:ok, %{}} = HD44780.I2C.execute(d, {:autoscroll, :off})
      assert {:ok, %{}} = HD44780.I2C.execute(d, {:autoscroll, :on})
      assert {:ok, %{}} = HD44780.I2C.execute(d, {:right_to_left, :on})
      assert {:ok, %{}} = HD44780.I2C.execute(d, {:left_to_right, :on})
      assert {:ok, %{}} = HD44780.I2C.execute(d, {:backlight, :off})
      assert {:ok, %{}} = HD44780.I2C.execute(d, {:backlight, :on})
    ...
    test "execute unsupported commands", %{display: d} do
      ...
      assert {:unsupported, %{}} = HD44780.I2C.execute(d, {:left_to_right, :off})
    ...
    test "change entry_mode", %{display: d} do
      assert {:ok, %{entry_mode: 4}} = HD44780.I2C.execute(d, {:right_to_left, :on})
      assert {:ok, %{entry_mode: 6}} = HD44780.I2C.execute(d, {:left_to_right, :on})
    end
